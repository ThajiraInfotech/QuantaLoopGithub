...
function TrustCheck({ children }: { children: ReactNode }) {
  return (
    <li className="flex shrink-0 items-center gap-2 text-small text-muted-foreground">
      <span
...
export async function LandingHero() {
  ...
  return (
    <section className="overflow-x-hidden border-b border-border bg-background">
      <div className="mx-auto grid w-full max-w-[86rem] items-center gap-8 px-5 py-10 sm:px-8 sm:py-12 lg:grid-cols-2 lg:gap-8 lg:px-10 lg:py-14 xl:gap-10">
        <div className="flex min-w-0 flex-col lg:pr-4">
          <p className="text-eyebrow">{t("eyebrow")}</p>

          <h1 className="mt-2 font-heading text-[1.75rem] font-bold leading-[1.1] tracking-[-0.03em] text-foreground sm:text-[2.125rem] lg:text-[2.5rem] xl:text-[2.75rem]">
            <span className="block">{t("titleLine1")}</span>
            <span className="block">{t("titleLine2")}</span>
          </h1>

          <div className="mt-4 space-y-2 sm:mt-5">
            <p className=" font-medium text-base leading-relaxed text-foreground sm:text-[1.0625rem] sm:leading-[1.6]">
              {t("description")}
            </p>
          ...
          <ul className="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 sm:mt-5 sm:grid-cols-2">
            {trustItems.map((item) => (
